...
# ── Human Insight endpoints ───────────────────────────────────────────────────

@router.post("/human-insight", response_model=HumanInsightOut, status_code=201)
async def generate_human_insight(
    body: HumanInsightRequest,
    user: dict = Depends(get_current_user),
    db: Client = Depends(_db),
):
    """
    Сүүлийн (эсвэл заасан) pattern_run-н pattern-уудыг LLM-ээр
    монгол хэлний энгийн текст болгоно. Үүсгэсэн insight-ийг
    human_insights хүснэгтэд хадгална.

    Нэг run-д хоёр дахь удаа дуудвал кэшлэгдсэн insight буцаана.
    """
    user_id = user["id"]
    run_id = body.pattern_run_id

    # Run_id өгөгдөөгүй бол сүүлийн run-г авна
    if not run_id:
        run_rows = (
            db.table("pattern_runs")
            .select("id")
        ...
        if not run_rows:
            raise HTTPException(
                status_code=404,
                detail="Pattern run олдсонгүй. Эхлээд тэмдэглэл бичнэ үү.",
            )
    ...
    if existing:
        _log.info(f"Human insight кэшээс буцаана: run_id={run_id}")
        return existing[0]

    # Тухайн run-н pattern-уудыг авна
    patterns = (
        db.table("detected_patterns")
        .select("pattern_type, pattern_data, strength_score")
        .eq("user_id", user_id)
    ...
    if not patterns:
        raise HTTPException(
            status_code=404,
            detail="Энэ run-д pattern олдсонгүй.",
        )

    # LLM дуудна
    llm = get_llm_service()
    try:
        result = await llm.generate_human_insight(patterns)
    except Exception as exc:
        _log.error(f"Human insight LLM алдаа: {exc}", exc_info=True)
        raise HTTPException(status_code=500, detail="Insight үүсгэхэд алдаа гарлаа")

    top_strength = max(p.get("strength_score") or 0 for p in patterns)
    ...
    _log.info(f"Human insight үүсгэлээ: user={user_id}, run={run_id}")
    return row
